...
var UsesDeclarationTransformer = /** @class */ (function (_super) {
    ...
    UsesDeclarationTransformer.prototype.transform = function (_value, node) {
        ...
        for (var _i = 0, usesArray_1 = usesArray; _i < usesArray_1.length; _i++) {
            ...
            var lib = libUnit.highLevel();
            if (!lib.isElement()) {
                continue;
            }
            var libAnnotations = lib.asElement().attributes(annotationsPropName);
            var libUsage = lib.asElement().attr(usagePropName);
            if (libUsage) {
            ...
            if (libAnnotations.length > 0) {
                ...
                for (var _a = 0, libAnnotations_1 = libAnnotations; _a < libAnnotations_1.length; _a++) {
                    ...
                    var aObj = this.dumper.dump(a);
                    if (!aObj || !aObj.name) {
                        continue;
                    }
                    var aName = aObj.name;
                    var range = a.property().range();
                    var patchedReference = this.getReferencePatcher().resolveReferenceValueBasic(aName, unit, resolver, [unit, libUnit], range);
                    if (!patchedReference) {
                        continue;
                    }
